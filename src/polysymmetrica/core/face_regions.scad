 ...
 * Returns: span-local unit ray `[0,+/-1,0]`
 */
function _ps_fr_span_face_ray(site) =
    [0, (site[17] < 0) ? -1 : 1, 0];

/**
 * Function: Build the anti-interference bisector direction in span-local coords.
 * Params: site (boundary-span site), eps (zero-length tolerance)
 * Returns: span-local unit direction between the filled-side face ray and adjacent-face plane ray
 */
function _ps_fr_span_bisector_dir_span_local(site, eps=1e-8) =
    let(
        face_ray = _ps_fr_span_face_ray(site),
        adj0 = site[18],
        adj_unit = (is_undef(adj0) || norm(adj0) <= eps) ? undef : v_norm(adj0),
        adj = is_undef(adj_unit)
            ? undef
            : (v_dot(adj_unit, face_ray) < 0 ? -adj_unit : adj_unit),
        raw = is_undef(adj) ? [0, 0, 1] : face_ray + adj
    )
 ...
 */
function _ps_fr_span_bisector_dir_local(site, eps=1e-8) =
    v_norm(_ps_fr_span_to_face_local(site, _ps_fr_span_bisector_dir_span_local(site, eps)));
 ...
 */
function _ps_fr_project_span_line(face_pts3d_local, site, z, max_project=undef, eps=1e-8) =
    let(
        seg3d = _ps_fr_span_seg3d(face_pts3d_local, site),
        mid = (seg3d[0] + seg3d[1]) / 2,
        dir = _ps_fr_span_bisector_dir_local(site, eps),
 ...
 * Params: face_pts3d_local (source face loop), loop_sites (sites for one boundary loop), loop_idx (loop id), z0/z1 (target Z planes), max_project (optional cap), eps (tolerance)
 * Returns: shell record `[points, faces, loop_idx, capped_count, bottom_loop2d, top_loop2d]`
 */
function _ps_fr_loop_shell(face_pts3d_local, loop_sites, loop_idx, z0, z1, max_project=undef, eps=1e-8) =
    let(
        lines0 = [for (site = loop_sites) _ps_fr_project_span_line(face_pts3d_local, site, z0, max_project, eps)],
        lines1 = [for (site = loop_sites) _ps_fr_project_span_line(face_pts3d_local, site, z1, max_project, eps)],
    ...
    [
        for (loop_idx = loop_ids)
            let(
                ...
                shell = _ps_fr_loop_shell(face_pts3d_local, loop_sites, loop_idx, z0, z1, max_project, eps)
            )
