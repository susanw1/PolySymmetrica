...
DEBUG_SHOW_MODEL = false;
DEBUG_SEG_LABELS = false;
DEBUG_SEG_EDGE_LABELS = false;
...
DEBUG_LIVE_FACE_SLAB = true;
DEBUG_LIVE_FACE_SLAB_FACES = [2,11];
DEBUG_LIVE_FACE_SLAB_CELLS = ["2/0"];
DEBUG_LIVE_FACE_SLAB_Z0 = BASE_Z;
DEBUG_LIVE_FACE_SLAB_Z1 = BASE_Z + FACE_T;
DEBUG_ATOMS = true;
DEBUG_ATOMS_SHOW_DISABLED = false;
DEBUG_ATOM_FACES = [2];
DEBUG_ATOM_CELLS = ["2/0"];
DEBUG_ATOM_DRAW_Z = BASE_Z + FACE_T + 0.35;
DEBUG_ATOM_T = 0.18;
DEBUG_SKIP_ATOM_INDICES = [2];
            ...
            place_on_face_visible_segments("nonzero", 1e-8, true) {
                if (
                    ...
                    ps_clip_to_visible_face_cell_ctx(
                        ...
                        band_z0 = band_z0,
                        band_z1 = band_z1,
                        debug_skip_atom_indices = DEBUG_SKIP_ATOM_INDICES
                    )
...
module debug_atoms(show_faces = undef, show_cells = undef) {
    atom_cols = [
        [1.0, 0.25, 0.25, 0.45],
        [0.25, 0.9, 0.25, 0.45],
        [0.25, 0.45, 1.0, 0.45],
        ...
        if (is_undef(show_faces) || len(search($ps_face_idx, [for (i=show_faces) i])) > 0) {
            place_on_face_visible_segments("nonzero", 1e-8, true) {
                if (
                    ...
                    atoms = _ps_fr_visible_cell_atoms(cell, 1e-8);

                    for (ai = [0:1:len(atoms)-1]) {
                        enabled = _ps_fr_atom_is_enabled(atoms[ai], ai, DEBUG_SKIP_ATOM_INDICES);
                        if (DEBUG_ATOMS_SHOW_DISABLED || enabled) {
                            apts = atoms[ai][0];
                            col = atom_cols[ai % len(atom_cols)];
                            draw_col = enabled ? col : [col[0], col[1], col[2], 0.12];
                            color(draw_col)
                            translate([0, 0, DEBUG_ATOM_DRAW_Z])
                                linear_extrude(DEBUG_ATOM_T)
                                    ps_polygon(points = apts, mode = "nonzero");

                            color(enabled ? "black" : "gray")
                            translate([
                                _centroid2(apts)[0],
                                _centroid2(apts)[1],
                                DEBUG_ATOM_DRAW_Z + DEBUG_ATOM_T
                            ])
                                linear_extrude(DEBUG_LABEL_T)
                                    text(
                                        enabled ? str("a", ai) : str("a", ai, " off"),
                                        size = DEBUG_EDGE_LABEL_SIZE,
                                        halign = "center",
                                        valign = "center"
                                    );
...
if (DEBUG_ATOMS) {
    debug_atoms(DEBUG_ATOM_FACES, DEBUG_ATOM_CELLS);
}
