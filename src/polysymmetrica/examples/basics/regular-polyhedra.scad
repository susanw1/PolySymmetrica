...
Examples use the place_on_*_ir() modules, in order to size the shapes equally. 
*/
...
translate([-100, 0, 0]) 
color("gold")
place_on_vertices_ir(tetrahedron(), IR) {
    cylinder(h = 5, r = 5, $fn = $ps_vertex_valence);
}

translate([0, 0, 0]) 
color("gold")
place_on_vertices_ir(octahedron(), IR) {
    cylinder(h = 5, r = 5, $fn = $ps_vertex_valence);
}

translate([100, 0, 0]) 
color("gold")
place_on_vertices_ir(icosahedron(), IR) {
    cylinder(h = 5, r = 5, $fn = $ps_vertex_valence);
}
...
translate([0, 100, 0]) 
color("gold")
place_on_vertices_ir(hexahedron(), IR) {
    cylinder(h = 5, r = 5, $fn = $ps_vertex_valence);
}

translate([100, 100, 0]) 
color("gold")
place_on_vertices_ir(dodecahedron(), IR) {
    cylinder(h = 5, r = 5, $fn = $ps_vertex_valence);
}
...
translate([0,0,DISPLAY_OFFSET_PLACE_ON_EDGES_EXAMPLE]) {
    translate([-100, 0, 0]) 
    color("silver")
    place_on_edges_ir(tetrahedron(), IR) {
        rotate([0,90,0]) translate([0,0,-$ps_edge_len/2]) cylinder(h = $ps_edge_len, r = 2, $fn = 20);
    }

    translate([0, 0, 0]) 
    color("silver")
    place_on_edges_ir(octahedron(), IR) {
        rotate([0,90,0]) translate([0,0,-$ps_edge_len/2]) cylinder(h = $ps_edge_len, r = 2, $fn = 20);
    }

    translate([100, 0, 0]) 
    color("silver")
    place_on_edges_ir(icosahedron(), IR) {
        rotate([0,90,0]) translate([0,0,-$ps_edge_len/2]) cylinder(h = $ps_edge_len, r = 2, $fn = 20);
    }
    ...
    translate([0, 100, 0]) 
    color("silver")
    place_on_edges_ir(hexahedron(), IR) {
        rotate([0,90,0]) translate([0,0,-$ps_edge_len/2]) cylinder(h = $ps_edge_len, r = 2, $fn = 20);
    }

    translate([100, 100, 0]) 
    color("silver")
    place_on_edges_ir(dodecahedron(), IR) {
        rotate([0,90,0]) translate([0,0,-$ps_edge_len/2]) cylinder(h = $ps_edge_len, r = 2, $fn = 20);
    }
}